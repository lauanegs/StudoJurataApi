...
10. **A maioria dos controllers expõe a entidade JPA diretamente.** A seção 2 descreve DTO de entrada/saída como padrão, mas ~24 dos 31 controllers recebem e devolvem a entidade (`AlunoController`, `TurmaController`, `NotaController`...); só o módulo de simulados e o de IA usam DTO + mapper. Migrar é mudança de contrato JSON em muitas rotas consumidas pelo front — tratar por recurso, com confirmação, nunca em lote silencioso.
11. **Rotas sem consumidor no front.** Várias rotas de CRUD genérico (`GET /pessoas/{id}`, `DELETE /simulados/{id}`, `/questao-aluno/**`, entre outras) não são chamadas por nenhuma tela. As que não tinham nenhum uso nem no front nem internamente (`/aula-conteudo/**`, `/questao-conteudo/**`, `/audit-log`, `/ia/historico-geracao/**` e rotas pontuais) foram removidas; as demais continuam como API, e removê-las é decisão de contrato.
